...
import { fetchPrestataire } from '@/services/prestataire';
...
export default {
  ...
  effects: {
    *fetchPecData({ payload }, { call, put }) {
      try {
        ...
        const { codePrestataire } = pec;
        const prestataire = yield call(fetchPrestataire, codePrestataire);
        yield put({ type: 'prestataireFetched', payload: prestataire });
  ...
  reducers: {
    ...
    prestataireFetched(state, action) {
      return {
        ...state,
